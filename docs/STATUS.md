...
- ✅ **M7** — the owner dashboard (back-office): `GET /api/leads` (decrypted phone/name/ALL answers; period/status incl. synthetic 'open'/flow filters; is_test excluded by default), `GET /api/dashboard` (funnel started/completed/abandoned/total, matches DB truth), `GET /api/conversations` + `POST .../status` (bot/human/closed) + `POST .../reply` (queued to outbox), `PUT /api/bot/publish` (reflected by GET /api/bot/settings). All six session-gated (401) + tenant-isolated (A never sees B). **Verify:** `tests/test_m7.bat` → M7 narrated **13/15**, strict `test_dashboard.py` **9 passed / 2 xfailed**, frontend `tsc --noEmit` clean, and M2 **12/12** + M3 + M4 + M5 (try-me 18/18 + sim 10/10) all still green. ⚠️ **KNOWN BUG (open):** `?period=week|month` 500s on `/api/leads` + `/api/dashboard` — `leads._period_clause` binds a `str` to a `$N::interval` asyncpg param (needs a `datetime.timedelta`); `period=all` (the default) works.
