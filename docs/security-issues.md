...
## M11.1 public booking polish — welcome generator + availability + service extras — audit 2026-06-21

> Scope: the M11.1 delta (decision 0012) —
> `supabase\migrations\0011_booking_service_extras.sql`;
> `backend\app\services\booking_welcome.py` (new), the welcome route + `compute_availability`
> additions in `backend\app\api\booking.py` / `backend\app\api\public_booking.py` /
> `backend\app\services\booking.py`; the `description`/`price`/`welcome_message` carry-through in
> `backend\app\models\booking.py`; and the frontend `frontend\src\components\booking\ServicesEditor.tsx`,
> `BookingSettingsPanel.tsx`, `BookingFlow.tsx`. Cross-checked against `bot_builder_ai.py`,
> `app\core\deps.py`, `app\core\config.py`, `0010_booking_slug_resolve.sql`, `app\api\me.py`.
> Read-only audit; no product code was modified.

**M11.1 contract checks (S1–S5) — verification result:**

- ✅ **S1 — `POST /api/booking/welcome/generate` gated by `current_business`; GEMINI key never logged/returned; generic 503/502.** Verified. The route lives on the admin `booking_router`, mounted under the `/api` group whose router-level `dependencies=[Depends(current_session)]` is the deny-by-default gate (`api/me.py:28,41`); it additionally takes `business_id = Depends(current_business)` (session-only, `deps.py:50-54`), so it is unreachable without a valid session (`api/booking.py:128-160`). The key is read inside `booking_welcome.get_gemini_client()` via `get_settings().gemini_api_key.get_secret_value()` and passed straight to the genai client — never logged, never put in a response model (`WelcomeGenerateResponse` carries only `message`, `models/booking.py:294-301`). Errors are generic and decoupled: `WelcomeNotConfiguredError → 503 "AI is not configured"`, `WelcomeGenerateError → 502 "AI is temporarily unavailable"` with `log.warning("welcome generate ai call failed")` (a static string, no key/output/`str(e)`); the service wraps the model call in `except Exception` and raises `WelcomeGenerateError("the AI model call failed") from exc` so no provider internals leak (`booking_welcome.py:101-133`, `api/booking.py:146-158`).
- ✅ **S2 — public `/availability`: slug resolved to a real business (404 else); from/to validated + range bounded (≤62 days); never logs PII; RLS-scoped read.** Verified. `public_availability` resolves the tenant ONLY via `_resolve_or_404` (the SECURITY DEFINER `resolve_booking_slug`, 404 on unknown — `public_booking.py:56-61,132-145`), never from path/query/body, then opens a normal RLS-scoped `tenant_connection` for the read (`public_booking.py:146-154`). `service_id` (≤64) and `from`/`to` (exactly length-10 query params) are bounded at the route signature (`public_booking.py:135-138`); `compute_availability` re-parses both with `strptime` (bad format → `InvalidDateRange`), rejects an inverted range, and BOUNDS the inclusive span to `AVAILABILITY_MAX_DAYS = 62` (else `InvalidDateRange`), all mapped to a generic 422 "invalid date range" (`booking.py:489-535`, `public_booking.py:155-160`). The day-by-day loop reuses `compute_slots` (RLS-scoped, unknown/inactive service → `[]`), so an oversized scan can't be driven. No PII exists on this path and none is logged.
- ✅ **S3 — `description`/`price`/`welcome_message` length/range-bounded (price ≥ 0); SQL parameterized; no injection.** Verified. At the model boundary (`extra="forbid"`): `description` ≤500 (`MAX_SERVICE_DESC_CHARS`), `welcome_message` ≤600 (`MAX_WELCOME_MESSAGE_CHARS`), `price: int|None = Field(..., ge=0)` so a negative price is rejected by Pydantic (`models/booking.py:42-43,115,159-173,291`). All three are bound as asyncpg `$n` params in `create_service`/`update_service`/`update_settings` (`booking.py:301-314,364-372,188-214`); the only dynamic SQL is the partial-update SET list built from a FIXED set of column-name literals with values still bound as params (`booking.py:342-372`) — no user value is interpolated. Nullable-clear is correct: the route forwards `set_description`/`set_price` from `body.model_fields_set` so an explicit `null` clears the column while an omitted field is untouched (`api/booking.py:210-223`, `booking.py:353-358`). The welcome prompt is text only — never executed, no eval/exec.
- ✅ **S4 — no change to tenant isolation: admin business_id from session only; foreign service/settings id → 404.** Verified. Every M11.1 admin touchpoint (welcome generate, services CRUD, settings get/put) takes `business_id = Depends(current_business)` and runs inside `tenant_connection(pool, business_id)` (`api/booking.py:67-244`); no delta model carries a `business_id` field. The new columns are additive on tables that already have RLS ENABLE+FORCE + USING/WITH CHECK + least-privilege grants (0009), which cover new columns automatically — the migration adds no table/policy/grant (`0011_booking_service_extras.sql:12-21`). `update_service`/`get_service`/`delete_service` keep `WHERE id = $1 AND business_id = $2`, so a foreign service_id matches zero rows → service returns None → route raises 404 (`booking.py:274-388`, `api/booking.py:224-243`). The public services/availability reads scope by the slug-resolved business under RLS. The new `welcome_message` on `PUT /booking/settings` persists via the existing tenant-scoped UPSERT with slug/timezone still server-owned (`booking.py:167-215`).
- ✅ **S5 — no secrets/PII in logs; welcome prompt carries only business name + active service names (no PII).** Verified. `generate_welcome_message` reads the business display name + ACTIVE service names RLS-scoped and passes ONLY `[s["name"] for s in services]` + the optional bounded `tone` into the prompt (`api/booking.py:141-147`); `build_welcome_prompt` interpolates only business name + service names + tone — no customer name/phone/email/notes ever reach it (`booking_welcome.py:70-96`). Customer PII columns are never read on the welcome/availability/services paths. Logging across the delta is static strings only (the one `log.warning` is "welcome generate ai call failed"); the key, prompt, and model output are never logged.

**Frontend (informational):**

- ✅ The owner panel sends only the optional `tone` to `POST /api/booking/welcome/generate` over the session cookie; the generated message is dropped into a draft textarea and only persisted on the explicit settings PUT (`BookingSettingsPanel.tsx:127-138,102-124`), client-truncated to 600 (server re-bounds). `ServicesEditor` parses price client-side (empty → null, else non-negative integer; negative/non-numeric rejected with a friendly error) and sends `null` to clear description/price — the server re-validates (`ServicesEditor.tsx:29-37,235-247`). `BookingFlow` in `preview` mode never fetches or POSTs (calendar/slots inert, submit disabled), so the owner preview can't leak or mutate anything; in `live` mode the tenant comes only from the verified slug, never a client business_id. All server text (welcome message, service name/description, business name) renders as auto-escaped JSX with `whitespace-pre-wrap` (no `dangerouslySetInnerHTML`) — no XSS sink. 503/502 map to friendly Hebrew, no raw error surfaced (`BookingSettingsPanel.tsx:36-42`).

**Overall verdict: SHIP — the M11.1 polish delta is tenant-correct, input-bounded, and leak-free.**
The welcome generator is session-gated, mirrors the proven `bot_builder_ai` validate-at-use plumbing
(503 with no key / 502 on failure), never logs or returns the GEMINI key, and feeds the model ONLY the
business name + active service names + an optional bounded tone — no customer PII. The public
`/availability` endpoint resolves the tenant solely from the slug (404 on unknown), runs RLS-scoped,
and bounds the date range to ≤62 days (inverted/oversized/bad-format → 422), so it can't be abused to
scan or hammer the DB. The three new columns are additive marketing/availability fields (not PII):
`price ≥ 0` and `description ≤500` / `welcome_message ≤600` are enforced at the Pydantic boundary and
bound as parameters, with correct nullable-clear semantics; no isolation, injection, or logging gap is
introduced. No new critical or medium issues. The two LOW notes below are abuse-hardening carry-overs
the delta touches, already tracked under M11.

### M11.1-01 — public READ endpoints (`/availability` per-day loop, `/slots`, `/services`) have NO rate limit, unlike CREATE — LOW (abuse hardening)
- **Where:** `backend\app\api\public_booking.py:101-176` (no `_check_rate_limit` on the GET routes) + `backend\app\services\booking.py:496-535` (the `/availability` day loop runs `get_service` + `get_settings` + `_bookings_on_local_day` per day, up to 62 days).
- **Why dangerous (mild):** The range bound (≤62 days) caps a SINGLE request, but the public READ endpoints have no rate limit (only the public CREATE does — `public_booking.py:78-93`). A scripted caller can repeatedly request a full 62-day window for a known slug, each driving ~62×3 small RLS-scoped queries — an availability/cost concern, NOT a confidentiality or tenant leak (every read is correctly scoped to the resolved tenant; no PII is exposed). This is the read-side companion to the pre-existing M11-02 note.
- **Fix direction:** Apply a lightweight per-(IP, slug) rate limit (or a short Redis cache of computed availability per service+month) to the public read endpoints, and/or a per-slug global cap, reusing the existing limiter helper. None blocks ship.

### M11.1-02 — `_client_ip` still trusts `X-Forwarded-For` first hop unconditionally (shared by the new flow) — LOW (carry-over from M11-02)
- **Where:** `backend\app\api\public_booking.py:64-77`.
- **Why dangerous (mild):** Unchanged from M11 — if the app is ever exposed without a trusted proxy that overwrites `X-Forwarded-For`, a client can forge the header to dodge the per-IP limiter. Not introduced by M11.1, but the new endpoints share this limiter design, so it is re-flagged. Not a leak.
- **Fix direction:** Honor `X-Forwarded-For` only behind a configured trusted-proxy count; otherwise use the peer address (`request.client.host`). Tracked under M11-02.

### M11.1 public booking polish — summary table
| ID | Severity | Area | One-line |
|----|----------|------|----------|
| S1–S5 | — | Verification | SHIP — welcome generator session-gated (`current_business`), GEMINI key never logged/returned, generic 503/502, prompt carries only business name + active service names (no PII); public `/availability` resolves tenant from the slug only (404 on unknown), RLS-scoped, range bounded ≤62 days (else 422); `price ≥ 0` / `description ≤500` / `welcome_message ≤600` enforced at the Pydantic boundary + bound as `$n` params (no injection, correct null-clear); no tenant-isolation change (admin business_id from session, foreign id → 404); no secret/PII in logs. |
| M11.1-01 | Low | Abuse / availability | Public READ endpoints have no rate limit (only CREATE does); a known slug can be scripted into repeated 62-day availability scans — add a per-(IP, slug) read limit or cache. Not a leak. Extends M11-02 to the read path. |
