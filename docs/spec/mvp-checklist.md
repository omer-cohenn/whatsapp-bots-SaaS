...
## M12 (platform-operator back-office) — ✅ DONE 2026-06-22
> Per [decision 0016](../decisions/0016-m12-back-office.md). Omer's mall control room: one window onto EVERY business + actions (change plan / suspend). The ONE surface that deliberately crosses the tenant wall, behind its own locked door (`ADMIN_EMAILS`, checked live). No tenant RLS is weakened.
- [x] **Migrations 0015–0017** (additive + idempotent): `plans` (global catalog, `app_role` SELECT only) + `subscriptions` (PK=business_id, **no** direct `app_role` grant — SD-only) + `businesses.is_active`; `usage_daily` (RLS **ENABLE+FORCE**, `p_tenant_isolation` on `current_business_id()`, `app_role` SELECT/INSERT/UPDATE) + `admin_audit` (SD-write only, **no** grant); five `SECURITY DEFINER` admin_* funcs, `REVOKE ALL FROM PUBLIC` + `GRANT EXECUTE TO app_role`, `search_path` pinned — the only cross-tenant doorway.
- [x] **Admin gate:** `ADMIN_EMAILS` (env, empty allowed, **not** fail-closed); `current_admin` = session + email∈ADMIN_EMAILS checked **LIVE** (→ 403 else, no stored session flag); `/api/admin/*` router carries `dependencies=[Depends(current_admin)]`; `/api/me` gains `is_admin` (computed live).
- [x] **Admin API** (session + admin gated): `GET /api/admin/overview` · `GET /api/admin/businesses?search=&limit=&offset=` · `GET /api/admin/businesses/{id}` · `GET /api/admin/businesses/{id}/usage?from=&to=` · `GET /api/admin/plans` · `PATCH /api/admin/businesses/{id}/subscription`. List/detail default plan `free`/status `active` with no subscription row; paging bounded (out-of-range → **422**); unknown id → **404**; set-subscription validates (junk status/plan → **422**, unknown business → **404**), syncs `businesses.is_active`, and writes an `admin_audit` row stamped with the real admin Google sub + email.
- [x] **Usage counters:** `usage.bump_safe` (UPSERT +1 on today's row, best-effort) wired at the webhook (msg_in/msg_out), lead create, public booking, and login.
- [x] **Suspend enforcement:** the webhook bot core reads `businesses.is_active` alongside `is_published` — a suspended business's bot stays **silent** (`{status:"suspended", replies:[]}`); re-activating restores answering.
- [x] **Frontend:** admin client/types, `isAdmin` in AuthContext, admin-only "ניהול" nav, `/admin` + `/admin/businesses` + `/admin/businesses/:id` pages + in-house SVG usage chart (tsc/lint/build pass).
- [x] 🚦 **Isolation re-proved:** `app_role` CANNOT directly SELECT `subscriptions`/`admin_audit` (permission denied) — the SD funcs are the only path; A can't read B's `usage_daily` (RLS holds, WITH-CHECK blocks a cross-tenant bump); no PII/secret in any admin response.

✅ **Done:** `tests/test_m12.bat` → M12 narrated **13/13** (incl. an active negative control: flip `is_active=false` by hand → bot silent → restore → answers) + strict `tests/test_m12.py` **26/26**; the strict M3–M11.2 + M6a/.1/.2 + M12 bundle **257 passed**; M2 **12/12** (no isolation regression). **QA regression fix (test-only):** `test_auth_gate.py::test_api_me_valid_session_returns_own_business` froze the OLD `/api/me` shape — updated to expect the contracted `is_admin` key (`False` for non-admin Avi). No product source was edited to pass — the locked M12 contract was already implemented across data/backend/frontend.

## M13 (back-office analytics + platform-level sales CRM) — ✅ DONE 2026-06-23
> Per [decision 0017](../decisions/0017-m13-backoffice-analytics-crm.md). The big extension of M12: a real SaaS-owner dashboard (LTV / messages-per-business / leads-by-type / AI-ops / by-plan / trends) **+ a platform-level sales CRM** (pipeline + notes). Still behind M12's one locked admin door; the tenant wall is not weakened.
- [x] **Migrations 0018–0020** (additive + idempotent): three operator-only tables with **no** direct `app_role`/`gateway_role` grant (SD-only, like `subscriptions`/`admin_audit`) — `platform_snapshots` (PK=day, daily aggregate for trend history), `business_crm` (PK=business_id, pipeline new→contacted→warming→won/lost + last/next-followup, `updated_at` trigger), `crm_notes` (per-business log, soft FK admin_user_id→users(id)); **12 `SECURITY DEFINER` funcs** (`admin_leads_by_type`/`admin_messages_by_business`/`admin_ai_ops_series`/`admin_by_plan`/`admin_trends_series`/`admin_business_extra`/`admin_ltv_summary`/`admin_crm_list`/`admin_set_crm_stage`/`admin_add_crm_note`/`admin_crm_notes`/`admin_upsert_today_snapshot`), each `REVOKE ALL FROM PUBLIC` + `GRANT EXECUTE TO app_role`, `search_path` pinned. LTV = plan price × tenure-months (floor 1; free→0) in SQL — an estimate, no real billing.
- [x] **`ai_call` usage metric:** `usage.METRIC_AI_CALL` + `bump_safe` after a successful Gemini call at the bot-builder endpoint (`api/bot_builder.py`) and the booking-welcome endpoint (`api/booking.py`).
- [x] **Admin analytics API** (session + admin gated): `GET /api/admin/analytics/leads-by-type?period=&plan=` · `…/messages?period=` · `…/ai-ops?from=&to=` · `…/by-plan?metric=&period=` · `…/trends?from=&to=`. Periods/metrics whitelisted (junk → **422**); bad dates → **422**.
- [x] **Admin CRM API:** `GET /api/admin/crm` (pipeline board) · `PATCH /api/admin/businesses/{id}/crm` (stage + optional follow-up; bad stage → **422**, unknown business → **404**) · `POST /api/admin/businesses/{id}/crm/notes` (→ **201**; blank note → **422**, unknown business → **404**) · `GET …/crm/notes` (newest-first). Every CRM write stamps the real admin Google sub + email into `admin_audit` + `crm_notes`.
- [x] **Extended M12 endpoints:** `GET /api/admin/overview` stamps today's `platform_snapshots` row (best-effort, idempotent) + adds `avg_ltv`/`total_ltv`; `GET /api/admin/businesses/{id}` adds `ltv_estimate`/`ai_calls`/nested `crm`.
- [x] **Frontend:** admin client/types extended, in-house SVG LineChart/DonutChart/StackedBarChart, enriched AdminHome, new `/admin/crm` pipeline board + `/admin/billing` messages-per-business, BusinessDetail LTV/ai_calls/CRM panel (tsc/lint/build pass).
- [x] 🚦 **Isolation re-proved:** `app_role` CANNOT directly SELECT `business_crm`/`crm_notes`/`platform_snapshots` (permission denied) — the SD funcs are the only path; A can't read B's `usage_daily`; no end-customer PII in any analytics/CRM response.

✅ **Done:** `tests/test_m13.bat` → M13 narrated **16/16** (incl. an active negative control: GRANT `app_role` a direct SELECT on `crm_notes` → read succeeds → REVOKE → denied again, restored) + strict `tests/test_m13.py` **30/30**; the strict M3–M12 + M13 bundle **287 passed**; M2 **12/12** (no isolation regression). **QA regression fix (test-only):** `test_m12.py::test_admin_responses_carry_no_lead_or_booking_content` froze the OLD detail-key allow-list — updated to include M13's additive `ltv_estimate`/`ai_calls`/`crm` keys + assert the nested `crm` block holds only `stage`/`last_contacted_at`/`next_followup_at` (no customer PII). No product source was edited to pass — the M13 contract was already implemented across data/backend/frontend.
