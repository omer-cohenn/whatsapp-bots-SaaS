...
if not exist "infra\.env" goto :gen_env
goto :start

:gen_env
echo [!] infra\.env not found - generating fresh secrets...
where python >nul 2>&1
if errorlevel 1 goto :no_python
python -c "import secrets,pathlib;t=lambda:secrets.token_urlsafe(32);pathlib.Path('infra/.env').write_text('GATEWAY_API_TOKEN='+t()+'\nBACKEND_WEBHOOK_URL=http://backend:8000/webhook/whatsapp\nPOSTGRES_USER=bizzup\nPOSTGRES_PASSWORD='+t()+'\nPOSTGRES_DB=bizzup\nREDIS_PASSWORD='+t()+'\n',encoding='utf-8')"
...
:no_python
echo [X] infra\.env is missing and Python was not found to generate it.
echo     Copy infra\.env.example to infra\.env, fill the values, then re-run.
goto :end
