"""M7 narrated — constants + narration + helpers + the test phases (split out of
the thin runner m7_full_test.py for length; same printed output, no secrets/PII).
The runner calls run_phases(...) here (passing the bookkeeping lists), reads tally()."""
...
async def run_phases(http, pool, redis, made_conv_ids, sids) -> None:
    """Run all M7 dashboard checks in order; append created conv-ids/sids for cleanup."""
    # ════════════════════════════════════════════════════════════════
    ...
    anon_transport = ASGITransport(app=app, raise_app_exceptions=False)
    async with AsyncClient(transport=anon_transport, base_url="http://test") as anon:
        codes = {
    ...
    sids.append(sid_b)
    bella_transport = ASGITransport(app=app, raise_app_exceptions=False)
    async with AsyncClient(transport=bella_transport, base_url="http://test") as bhttp:
